...
void gmm_new(GMM *gmm, int M, int D, int frames, const char *cov_type)
{
	//GMM *gmm;
	//GMM gmm[1];
...
//	gmm->weights = (double*)malloc(sizeof(double) * M);
//	gmm->means = (double*)malloc(sizeof(double) * M * D);
//	gmm->covars = (double*)malloc(sizeof(double) * M * D);

	gmm->P_k_giv_xt = (double*)malloc(sizeof(double) * frames * D);
...
double gmm_score(GMM *gmm, const double *X, int N)
{
	int i, n;
	//double temp[3]={0};
	double *temp = (double*)malloc(sizeof(double)*gmm->M);
	for(i=0; i< gmm->M; i++) {
		temp[i] = 0;
	}
/*
...
*/
	...
	for (n=0; n<N; n++){
		for(i=0; i < gmm->M; i++) {
//			gmm->P_k_giv_xt[i] = (gmm->P_k_giv_xt[i] * n + gmm->P_k_giv_xt[gmm->M * n + i]) / (n+1);
			temp[i] += gmm->P_k_giv_xt[gmm->M*n+i];
		}
		IPrintf("%d: Duck %lf, Dove %lf, Bluejay %lf\n", n, gmm->P_k_giv_xt[0*N+n], gmm->P_k_giv_xt[1*N+n], gmm->P_k_giv_xt[2*N+n]);
	}
//	for(i=0; i < gmm->M; i++) {
//		printf("P%d %lf ", i, temp[i]/N);
//	}
	for(i=0; i < gmm->M; i++) {
		gmm->P_k_giv_xt[i] = temp[i]/N;
	}
//	printf("\n");
...
double _gmm_compute_membership_prob(GMM *gmm, const double *X, int N)
{
	...
	double llh = 0;
	double llh_t = 0;
	double *temp = malloc(sizeof(double)*gmm->M*N);
//	double temp[200] = {0};
	double temp2=1;
...
//			replace all k*N + t with k*t + k
			temp[gmm->M*t + k] = log(gmm->weights[k]) + _gmm_log_gaussian_pdf(&X[gmm->D*t], &gmm->means[k*gmm->D], &gmm->covars[k*gmm->D], gmm->D, gmm->cov_type);
			if (max == -1 || temp[gmm->M*t + k] > max)
				max = temp[gmm->M*t + k];
		}

		llh_t = 0;
		for (k=0; k<gmm->M; k++)
			llh_t += exp(temp[gmm->M*t + k] - max);
		llh_t = max + log(llh_t);

		for (k = 0; k < gmm->M; k++)
		{
			temp2 = temp[gmm->M*t + k] - llh_t;
			if(temp2>1) temp2 = 1;
			gmm->P_k_giv_xt[gmm->M*t + k] = exp(temp2);
		}
